...
def main() -> None:
    ...
    emit(f"VRAM type (amd-smi):                {peaks.vram_type!r}")
    emit(f"VRAM technology (classified):       {peaks.vram_tech!r}")
    emit(f"Effective-rate factor (from tech):  {peaks.ddr_factor!r}")
    emit()
    ...
    if peaks.bandwidth_source == "runtime":
        ...
        emit("  - Roughly MATCHES (within ~1-2%) -> trust it.")
        if peaks.vram_tech == "hbm":
            emit("    (HBM uses a textbook DDR x2 factor -- this should match closely")
            emit("    on any HBM card, not just the one this was confirmed against.)")
        elif peaks.vram_tech == "gddr6":
            emit("    (GDDR6 uses an EMPIRICAL x17.8 factor confirmed against exactly")
            emit("    ONE real card -- RX 7900 XTX/gfx1100. If it matches here too on a")
            emit("    DIFFERENT GDDR6 card/bus-width, that's a valuable second data")
            emit("    point confirming the factor generalizes -- worth noting.)")
        emit("  - Doesn't match -> one of these is off for this specific card:")
        emit("    - mem_clock_mhz/bus_width_bits parsed wrong -> check the raw amd-smi")
        emit("      output in the full log below against the key paths tried in")
        emit("      _query_amd_smi_mem_bus_specs() (src/tools/execution.py).")
        if peaks.vram_tech == "gddr6":
            emit("    - OR the GDDR6 x17.8 factor doesn't generalize to this card's")
            emit("      specific clock-reporting behavior -- this factor is explicitly")
            emit("      provisional (see _MEM_TECH_DDR_FACTOR's 'gddr6' comment). If")
            emit("      you have another confirmed real GDDR6 card's numbers, that's")
            emit("      exactly the second data point needed to refine or confirm it.")
        elif peaks.vram_tech is None:
            emit("    - vram type wasn't recognized (see VRAM type above) -> no factor")
            emit("      was applied at all if bandwidth is None above; if a technology")
            emit("      name did print but isn't 'hbm'/'gddr6', add it to")
            emit("      _classify_vram_technology()/_MEM_TECH_DDR_FACTOR once confirmed.")
